...
- Ordinary `PlayerView.timeline` must not fall back to `run.eventHistory`. Raw event history may remain in backend/GM/debug state, but if no LifeNodes are available the ordinary timeline is empty/safe rather than rendering raw AI text.
...
- Annual Year Tick v2 must replace the real annual generation path, not sit beside it. Play-session annual branches, including the first playable branch after opening and async provider fallback paths, must call `buildNextEventContract`, validate with `assertStoryContract`, and apply `applyAnnualFactPackageToResponse` before the response is committed.
- Topic Ledger is a selection authority, not only a validator report. If a candidate annual topic repeats a recent topic family, arena, object focus, or pressure type inside its cooldown window, the annual focus chooser must try another curriculum/topic candidate before accepting the year.
...
- Treat old-asset budgets as hard scene contracts. Recurring assets such as jade tokens, back mountains, white deer, old booklets, scripture pavilions, mines, sects, and beasts can receive `maxSentences`, `cannotOpenScene`, `cannotDriveChoices`, and `mainPressureAllowed`; scene validation must reject over-budget usage.
- Carry old-asset budgets through the real annual player path. `AnnualFactPackage.assetRoles` must be converted into `AnnualAgenda.storyAssetBudget`, preserved through story-state normalization, rendered to providers only as sanitized player-visible constraints, enforced by `SceneComplianceValidator` and `LifeNodeValidator`, and rejected by Player Surface projection before ordinary `PlayerView` can display polluted LifeNodes.
