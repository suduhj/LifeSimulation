...
Ordinary browser panels should read the Selector Graph output exposed as `panelViews`, not recompute their own view model from scattered `run` fields. The main panel reads `panelViews.main`, the attribute panel reads `panelViews.attributes`, and the story/timeline panel reads `panelViews.story`. Legacy `run` fields can remain as compatibility fallback; new ordinary UI work should treat selector views as the rendering contract. The ordinary attribute panel is a growth-manifestation dashboard: selector output must already group talent-core attributes, fate-foundation attributes, and cultivation/world progress, including current performance, potential, manifestation progress, age-sealed potential, and exposure labels.
...
- Use the Yearly Outcome Ledger as the system-owned annual settlement layer. AnnualAgenda controls what AI should render; `YearlyOutcome` records what the engine recognizes for that year: curriculum result, human-life outcome, growth impact, topic impact, and panel-display intent. `YearlyOutcome` records must become `annual.outcome_recorded` DomainEvents and survive replay.
- Annual growth should be engine-settled. A normal annual branch may realize existing potential or increase exposure through the Yearly Outcome mapping even when the AI returns no growth patch. Normal years should not casually increase total potential; reserve potential changes for explicit rules or exceptional outcomes.
...
- Annual outcome contract when available: curriculum slot, required human delta, expected growth impact, topic impact, and panel intent. The AI should write prose and choices that fit this contract, but the engine owns the final `YearlyOutcome` and its DomainEvents.
...
- Regression tests for Yearly Outcome Ledger: annual advancement should produce growth/exposure impact even when AI omits growth evidence, `YearlyOutcome` records must enter `eventLog` as `annual.outcome_recorded`, replay must preserve them, the main summary must not show pressure/score bookkeeping, and attribute panels must read Growth Ledger selector values.
