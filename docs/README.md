...
- [Spec-to-Proof Operating System](./spec-to-proof-operating-system.md): mandatory Proof Contract, death-test, and evidence-gate workflow for non-trivial fixes and repeated bugs.
...
Key current architecture requirement: continuity-critical story facts live in structured `worldState.storyState`, not in AI prose. Player actions are converted into intent and simulation outcomes first; those outcomes can raise pressure on five lightweight world axes: life pressure, talent manifestation, NPC relationships, world opportunity, and choice consequence. Cross-year branches are directed by an engine-owned annual fact package before AI/mock rendering. The annual package chooses a primary axis and secondary axis, supplies the year's primary life delta, and keeps ongoing threads as background pressure, while the validator rejects reopened closed facts, forbidden scene skeletons, and forbidden yearly event shapes. Yearly Outcome Ledger records the system-owned annual result, maps curriculum slots into growth/exposure impact, and sends those changes through DomainEvents so `player.growthLedger` and `panelViews.attributes` show the same state. Canonical Life Runtime records accepted prose as `mvp.life_node.v1` via `life.node_recorded`; ordinary story panels read those LifeNodes through Player Surface and show age/body entries rather than AI event titles. Opening Origin Ledger, Story Asset Lifecycle, and Player Experience Director add early-life variation, named-asset cooldowns, and player-visible rhythm balancing without letting UI or AI own state. Attribute growth is also engine-owned: `player.growthLedger` stores potential, realized value, current effective value, maturity cap, locked potential, evidence, milestones, and exposure; AI receives capability packages and may submit only growth evidence, not direct effective power. Accepted state changes now pass through the Event-Sourced Life Runtime: `statePatch` becomes DomainEvents, reducers settle the run projection, invariants guard the transition, and `eventLog` is replayable save authority. Read surfaces are split by audience: ordinary browser APIs return only `sessionId` plus safe `PlayerView`; PromptView feeds AI context; GM/debug surfaces may inspect raw state. The Selector Graph still derives `panelViews.main`, `panelViews.attributes`, and `panelViews.story`, but ordinary UI receives them only inside `playerView.panels`. Details live in [Technical Standards](./technical-standards.md), [MVP Program Skeleton](./mvp-program-skeleton.md), [AI Output Protocol](./ai-output-protocol.md), and [Potential, Manifestation, And Exposure Rules](./potential-manifestation-exposure-rules.md).
