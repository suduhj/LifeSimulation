...
- After the player clicks `开始人生`, the formal life page starts from age 0 and displays early-years nodes from `PlayerView.timeline` / per-age `opening_year` LifeNodes in strict age order: 0岁, 1岁, 2岁, and so on until the first meaningful branch. Each node must match that age and contain meaningful life texture, not wrong-age content or empty "learned to walk" filler. Ordinary UI must not reconstruct this sequence from fixed fallback templates.
