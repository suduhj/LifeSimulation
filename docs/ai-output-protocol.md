...
- Early-years auto progression may be stored in `statePatch.worldStateChanges` as `opening.earlyLifeTimeline` for backend/debug compatibility, but ordinary player timelines must be projected from per-age `opening_year` LifeNodes recorded through `life.node_recorded`. The LifeNodes must be age-ordered from age 0 to `firstActionAge - 1` with no missing, duplicated, or skipped ages. Each node body must match its own age and avoid mentioning later ages as if they had already happened. The writing should still avoid empty filler; early years are life undertone, family relationships, talent manifestation, and world rules.
...
- formal life timeline: readable per-age `opening_year` LifeNodes from `PlayerView.timeline` in strict age order from age 0 to the first action age, followed by the first playable branch at the bottom current node. The ordinary web UI must not rebuild these nodes from legacy opening fallback templates.
