...
16. Build a Yearly Outcome Ledger record for each annual branch. The Annual Agenda controls the writing contract; the Yearly Outcome records the system-approved curriculum result, human outcome, realized growth, exposure change, topic impact, and player-summary mode.
17. Persist annual curriculum, topic, agenda, and outcome data through `statePatch -> DomainEvents -> reducer -> run projection`: `storyState.curriculum`, `storyState.topicLedger`, `storyState.annualAgendas`, and `storyState.yearlyOutcomes` must survive save/load replay.
18. Generate the next playable life event from a narrative director contract. The annual fact package owns the year's main life delta, curriculum slot, and selected axes; tracked threads such as clues, family pressure, institutions, resources, and world danger can support it as background instead of replaying a settled discovery or repeated scene structure.
19. Save and load portable run JSON with MVP run-state validation. Current saves include `eventLog`; load prefers `replayRun(eventLog)` over trusting the snapshot.
20. Build PlayerView, PromptView, and GMView projections from the authoritative run so ordinary UI, AI prompts, and debug tools do not consume the same raw surface.
21. Build Selector Graph panel views from the authoritative run: `panelViews.main`, `panelViews.attributes`, and `panelViews.story` are the browser panel contract, while raw run internals stay compatibility/debug data.
22. Run a developer-facing command-line setup, event loop, status summary display, and MVP short-run ending summary.
23. Start every new life with a non-interactive opening sequence: birth background card, fate preview, and automatic early-year progression to the first meaningful branch.
...
Ordinary browser panels use `panelViews` from the Selector Graph. Main status should read `panelViews.main`; the growth-manifestation attribute dashboard should read `panelViews.attributes`; story/timeline support data should read `panelViews.story`. The web client may keep legacy `run` fallbacks for older payloads, but new ordinary UI code should not recompute panel state from `player.growthLedger`, raw `storyState`, or event-log internals.

Attribute displays must use player-facing Growth Ledger language: `当前表现` for current effective ability, `显化进度` for realized growth versus potential, `潜质上限` for total potential, `年龄封存` for potential still held back by age/body/training/story conditions, and `外界关注` for exposure/attention. `panelViews.attributes.groups` should present talent-core attributes, fate-foundation attributes, and cultivation/world progress as grouped cards. Ordinary UI should not show raw backend labels such as `effective`, `realized`, `maturityCap`, `lockedPotential`, or `growthLedger`.
...
- Ordinary UI receives PlayerView and Selector Graph `panelViews` alongside legacy compatibility fields. New ordinary display work should read `panelViews` for page panels and PlayerView for compact player-safe data; raw run internals belong to GM/debug surfaces. The attribute panel now renders `panelViews.attributes.groups` as growth-manifestation cards, so UI code should not separately calculate peer labels, manifestation ratios, age-sealed potential, or cultivation progress.
...
- Yearly Outcome Ledger v1 settles annual branches after prose validation. Even if AI omits growth evidence, the engine maps the curriculum slot to approved realized growth or exposure changes, records a `YearlyOutcome`, emits `annual.outcome_recorded`, and keeps ordinary main-panel summaries focused on character/world/growth stage/core talent instead of pressure or score bookkeeping.
