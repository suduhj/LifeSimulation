...
- Yearly Outcome Ledger is the annual result authority. Annual agendas must produce `annual.outcome_recorded`; curriculum growth/exposure impact must become growth/exposure DomainEvents; Growth Ledger and `panelViews.attributes` must reflect the resulting values after replay. User-facing annual generation paths in `play-session.js` must use the contracted annual wrapper before `applyAiResponseToRun()` so a raw AI/mock `statePatch` cannot advance a year without a system-owned YearlyOutcome.
