...
12. Generate the next playable life event from a narrative director contract when a tracked thread has a required next pressure, so AI/mock text advances the thread instead of reopening a settled discovery.
13. Save and load portable run JSON with MVP run-state validation.
14. Run a developer-facing command-line setup, event loop, status summary display, and MVP short-run ending summary.
15. Start every new life with a non-interactive opening sequence: birth background card, fate preview, and automatic early-year progression to the first meaningful branch.
...
- `src/narrative-director.js`: converts story state into an event contract for the next branch when a tracked thread must advance.
- `src/story-contract-validator.js`: rejects generated events that reopen closed facts or repeat forbidden scene skeletons.
...
- The current state-first MVP protects the cultivation jade-talisman/lingyinfu thread from repeated first-discovery loops. When the thread is identified, the next event must advance family pressure around the stored object and mountain pull instead of rediscovering the same object and footsteps.
