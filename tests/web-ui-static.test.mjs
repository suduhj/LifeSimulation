...
describe("web UI contract", () => {
  ...
  it("keeps fate preview spoiler-safe while exposing Chinese talent details on demand", () => {
    ...
    assert.doesNotMatch(app, /const cultivationAttrLabels/);
    assert.doesNotMatch(app, /appearance: "仙姿"/);
    assert.doesNotMatch(app, /\$\{worldSpecificLabel\}（\$\{baseLabel\}）/);
    assert.match(app, /function fateAttributeSealText/);
    ...
    assert.match(app, /经验封存 \$\{lockedPotential\}/);
    assert.match(app, /家庭底色/);
    assert.match(app, /机缘倾向/);
